...
if ($acao == 'index' && $parametro == '') {
    ...
    for ($i = 0; $i < count($obj); $i++) {
        ...
        $sql = $db->prepare("SELECT operadores.matricula, usuarios.nome, operadorequipamento.tagequipamento, operadorequipamento.localtrabalho, operadorequipamento.atividade, operadorequipamento.transporte FROM operadorequipamento, operadores, usuarios where operadores.matricula = usuarios.matricula and operadores.matricula = operadorequipamento.matricula and operadorequipamento.idlista = ?");
...
if ($acao == 'show' && $parametro != '') {
    ...
    $sql = $db->prepare("SELECT operadores.matricula, operadores.nome, operadorequipamento.tagequipamento, operadorequipamento.localtrabalho, operadorequipamento.atividade, operadorequipamento.transporte FROM operadorequipamento, operadores where operadores.matricula = operadorequipamento.matricula and operadorequipamento.idlista = ?");
    $sql->execute([$parametro]);
    $escala = $sql->fetchAll(PDO::FETCH_ASSOC);
